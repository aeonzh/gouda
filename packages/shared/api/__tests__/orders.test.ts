...
function createThenable<T>(result: { data: T; error: any }) {
  const qb: any = {
    ...
    then: (onFulfilled: (v: typeof result) => any, onRejected?: (e: any) => any) =>
...
describe('orders API', () => {
  ...
  it('getCartItems maps product:products alias to product and handles array', async () => {
    jest.isolateModules(() => {
      ...
      jest.doMock('../supabase', () => ({ getSupabase: () => client, supabase: client }));
  ...
  it('createOrderFromCart uses price_at_time_of_order and clears cart', async () => {
    jest.isolateModules(() => {
      ...
      const cartItems = [
        ...
        { product_id: 'p2', quantity: 1, price_at_time_of_add: 10, carts: cart },
      ] as any;
