...
CREATE POLICY "Business users can view profiles for their business" ON profiles FOR SELECT USING (
...
CREATE POLICY "Business users can create profiles for their business" ON profiles FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.members m_auth WHERE m_auth.profile_id = auth.uid() AND m_auth.role_in_business IN ('owner', 'sales_agent') AND EXISTS (SELECT 1 FROM public.members m_target WHERE m_target.profile_id = profiles.id AND m_target.business_id = m_auth.business_id))
);
CREATE POLICY "Business users can update profiles for their business" ON profiles FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.members m_auth WHERE m_auth.profile_id = auth.uid() AND m_auth.role_in_business IN ('owner', 'sales_agent') AND EXISTS (SELECT 1 FROM public.members m_target WHERE m_target.profile_id = profiles.id AND m_target.business_id = m_auth.business_id))
);

-- RLS Policies for products table
CREATE POLICY "Admins can view all products" ON products FOR ALL USING (auth.jwt() ->> 'user_role' = 'admin');
CREATE POLICY "Business users can view products for their business" ON products FOR SELECT USING (
...
CREATE POLICY "Business users can add products to their business" ON products FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.members m WHERE m.profile_id = auth.uid() AND m.business_id = products.business_id AND m.role_in_business IN ('owner', 'sales_agent'))
);
CREATE POLICY "Business users can update products in their business" ON products FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.members m WHERE m.profile_id = auth.uid() AND m.business_id = products.business_id AND m.role_in_business IN ('owner', 'sales_agent'))
);
...
CREATE POLICY "Business users can view categories for their business" ON categories FOR SELECT USING (
...
CREATE POLICY "Business users can view carts for their business" ON carts FOR SELECT USING (
...
CREATE POLICY "Business users can create carts for a customer within their business" ON carts FOR INSERT WITH CHECK (
...
CREATE POLICY "Business users can update carts for their business" ON carts FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.members m WHERE m.profile_id = auth.uid() AND m.business_id = carts.business_id AND m.role_in_business IN ('owner', 'sales_agent'))
);
...
CREATE POLICY "Business users can view cart items for carts for their business" ON cart_items FOR SELECT USING (
...
CREATE POLICY "Business users can manage cart items for carts for their business" ON cart_items FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.carts c JOIN public.members m ON c.business_id = m.business_id WHERE c.id = cart_id AND m.profile_id = auth.uid() AND m.role_in_business IN ('owner', 'sales_agent'))
);
CREATE POLICY "Business users can update cart items for carts for their business" ON cart_items FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.carts c JOIN public.members m ON c.business_id = m.business_id WHERE c.id = cart_id AND m.profile_id = auth.uid() AND m.role_in_business IN ('owner', 'sales_agent'))
);

-- RLS Policies for orders table
CREATE POLICY "Admins can view all orders" ON orders FOR ALL USING (auth.jwt() ->> 'user_role' = 'admin');
CREATE POLICY "Business users can view orders for their business" ON orders FOR SELECT USING (
...
CREATE POLICY "Business users can create orders for their business" ON orders FOR INSERT WITH CHECK (
...
CREATE POLICY "Business users can update orders for their business" ON orders FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.members m WHERE m.profile_id = auth.uid() AND m.business_id = orders.business_id AND m.role_in_business IN ('owner', 'sales_agent'))
);
...
CREATE POLICY "Business users can view order items for their business" ON order_items FOR SELECT USING (
...
CREATE POLICY "Business users can create order items for their business" ON order_items FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM public.orders o JOIN public.members m ON o.business_id = m.business_id WHERE o.id = order_items.order_id AND m.profile_id = auth.uid() AND m.role_in_business IN ('owner', 'sales_agent'))
);
CREATE POLICY "Business users can update order items for their business" ON order_items FOR UPDATE USING (
    EXISTS (SELECT 1 FROM public.orders o JOIN public.members m ON o.business_id = m.business_id WHERE o.id = order_items.order_id AND m.profile_id = auth.uid() AND m.role_in_business IN ('owner', 'sales_agent'))
);
...
CREATE POLICY "Business users can view their own organisation" ON organisations FOR SELECT USING (
...
CREATE POLICY "Business users can create members within their business" ON members FOR INSERT WITH CHECK (
