...
CREATE OR REPLACE FUNCTION public.get_auth_user_business_ids()
RETURNS SETOF uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
GRANT EXECUTE ON FUNCTION public.get_auth_user_business_ids() TO authenticated;
...
CREATE POLICY "profiles_select_access" ON profiles FOR SELECT USING (
    deleted_at IS NULL AND (
        (public.get_user_role_by_id((select auth.uid())) = 'admin') OR
        ((select auth.uid()) = id) OR
        (EXISTS (
            SELECT 1
            FROM public.members m_target
            WHERE
                m_target.profile_id = profiles.id AND
                m_target.business_id IN (SELECT public.get_auth_user_business_ids())
        ))
    )
);
